...
printf "Configuring UART in /dev/ttyUSB0 to 19200bps 8n1... \n"
stty -F /dev/ttyUSB0 19200 cs8 -cstopb -parenb raw
...
# Scope capture start
# Start a 1M Samples, 24MHz capture with sigrok in the background (eventually outputs a file called $INDEX); store its PID to wait for it later
sigrok-cli --driver=hantek-6xxx --config samplerate=$SAMPLERATE --channels CH1,CH2  --samples=$SAMPLES --output-file $CAPTUREFOLDER/$INDEX.zip &
PIDsigrok=$!
...
# Note that we send an additional 'e' letter to the target to encrypt
